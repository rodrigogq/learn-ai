...
Here is that idea made concrete, because it is the very same mechanism that will make training work. Take one \$1 bet on a single number in American roulette. The wheel has 38 slots. If your number hits (probability $\frac{1}{38}$) you win \$35; otherwise (probability $\frac{37}{38}$) you lose your \$1. The player's expected value per \$1 bet is:

$$\mathbb{E}[\text{player}] = \frac{1}{38}\cdot(+35) \;+\; \frac{37}{38}\cdot(-1) = \frac{35 - 37}{38} = -\frac{2}{38} \approx -\$0.053$$

So every dollar bet is worth about **−5.3 cents to the player**, which is the same as **+5.3 cents to the casino**. On any *single* spin this number is invisible — the player might walk away with \$35. But now bring back Section 2: over millions of independent bets, the law of large numbers drags the *average* result onto that expected value with iron reliability. So the casino's income is not luck, it is arithmetic — roughly the number of bets times the expected value per bet. A casino that takes a million \$1 bets expects about **\$53,000** of profit (a million times 5.3 cents), and the *more* bets it processes, the *less* that figure wobbles. That is exactly why casinos love volume and enforce betting limits: volume turns a tiny per-bet expected value into near-certain revenue. The expected value **is** the business model, printed right on the felt.
