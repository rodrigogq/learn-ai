...
**Where are the biases?** Every neuron in the network — all four — has **its own bias**: a single constant added inside its tanh, on top of the weighted sum. They are **not** shared. In the diagram they are the orange arrows — `b₁`, `b₂`, `b₃`, one per hidden neuron, and `b` for the output neuron: **four biases in total**. That is exactly why each hidden neuron is *three* numbers (2 input weights **+ its own bias**) and the output neuron is *four* (3 weights + its bias). What does a bias do? Chapter 7 showed it: the weights set the *tilt* of a neuron's dividing line, and the bias sets *where* that line sits — without it, every line would be forced through the origin. So a bias is just as much a learned parameter as any weight; gradient descent adjusts all 13 numbers, biases included.
